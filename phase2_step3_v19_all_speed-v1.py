...
# Define output path
output_path = "/Users/hamid/Documents/CO2 PAPER FINAL/PHASE_2_graphs/phase2-step1_all_speed.xlsx"
...
with pd.ExcelWriter(output_path, engine='xlsxwriter') as writer:
    ...
    chart_fc = workbook.add_chart({'type': 'line'})
    for scenario in ["Original Scenario", "Paint (5%) Scenario", "Bulbous Bow Scenario", "Combined Paint and Bulbous Bow Scenario"]:
        # Rows for each scenario (10 speeds per scenario)
        start_row = 1 + ["Original Scenario", "Paint (5%) Scenario", "Bulbous Bow Scenario", "Combined Paint and Bulbous Bow Scenario"].index(scenario) * 10
        end_row = start_row + 9
        chart_fc.add_series({
            'name': scenario,
            'categories': f'=Comparison_All_Speeds!$B${start_row + 1}:$B${end_row + 1}',
            'values': f'=Comparison_All_Speeds!$C${start_row + 1}:$C${end_row + 1}',
            'line': {'width': 2}
        })
    ...
    chart_co2 = workbook.add_chart({'type': 'line'})
    for scenario in ["Original Scenario", "Paint (5%) Scenario", "Bulbous Bow Scenario", "Combined Paint and Bulbous Bow Scenario"]:
        start_row = 1 + ["Original Scenario", "Paint (5%) Scenario", "Bulbous Bow Scenario", "Combined Paint and Bulbous Bow Scenario"].index(scenario) * 10
        end_row = start_row + 9
        chart_co2.add_series({
            'name': scenario,
            'categories': f'=Comparison_All_Speeds!$B${start_row + 1}:$B${end_row + 1}',
            'values': f'=Comparison_All_Speeds!$D${start_row + 1}:$D${end_row + 1}',
            'line': {'width': 2}
        })
