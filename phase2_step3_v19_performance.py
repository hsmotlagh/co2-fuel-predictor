...
# Define output path
output_path = "/Users/hamid/Documents/CO2 PAPER FINAL/PHASE_2_graphs/phase2-step1_performance_analysis.xlsx"
...
for speed in performance_df["Ship Speed (knots)"].unique():
    ...
    original_eta = df_speed[df_speed["Scenario"] == "Original Scenario"]["ηO"].iloc[0]
    ...
    for _, row in df_speed.iterrows():
        ...
        eta_increase = ((row["ηO"] - original_eta) / original_eta) * 100
        treq_reduction = ((original_treq - row["Treq (kN)"]) / original_treq) * 100
        efficiency_data.append([
            row["Scenario"], row["Ship Speed (knots)"], fc_reduction, eta_increase, treq_reduction
        ])

# Create efficiency DataFrame
efficiency_df = pd.DataFrame(efficiency_data,
                             columns=["Scenario", "Ship Speed (knots)", "FC Reduction (%)", "ηO Increase (%)",
                                      "Treq Reduction (%)"])
...
with pd.ExcelWriter(output_path, engine='xlsxwriter') as writer:
    ...
    # Pivot data for chart (average efficiency gains across speeds)
    pivot_df = efficiency_df.pivot_table(
        index="Scenario",
        values=["FC Reduction (%)", "ηO Increase (%)", "Treq Reduction (%)"],
        aggfunc='mean'
    )
    pivot_df.to_excel(writer, sheet_name='Pivot_Data', startrow=0)

    # Bar chart for efficiency gains
    chart = workbook.add_chart({'type': 'column'})
    metrics = ["FC Reduction (%)", "ηO Increase (%)", "Treq Reduction (%)"]
    colors = ['#1F77B4', '#2CA02C', '#D62728']  # Blue, Green, Red

    for i, metric in enumerate(metrics):
        col = chr(66 + i)  # B, C, D for FC, ηO, Treq
        chart.add_series({
            'name': metric,
            'categories': '=Pivot_Data!$A$2:$A$7',
            'values': f'=Pivot_Data!${col}$2:${col}$7',
            'fill': {'color': colors[i]},
            'gap': 20
        })

    chart.set_title({'name': 'Scenario-Specific Performance Analysis'})
    chart.set_x_axis({'name': 'Scenario'})
    chart.set_y_axis({'name': 'Efficiency Gain (%)', 'major_gridlines': {'visible': True}, 'min': 0, 'max': 30})
