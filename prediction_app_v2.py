...
# so that PyInstaller can find these files when you bundle
if getattr(sys, 'frozen', False):
    # running in a PyInstaller bundle
    base_path = sys._MEIPASS
else:
    # running in the normal interpreter
...
class PredictionApp:
    ...
    def _build_prediction_tab(self):
        ...
        # buttons
        bf = ttk.Frame(frm)
        bf.grid(row=len(RANGES),column=0,columnspan=3,pady=5)
    ...
    def _plot_results(self, vals, fcp, co2p):
        ...
        for name in sheet_names:
            sd=excel_data[name]
            self.ax1.plot(sd['Ship Speed (knots)'],
                          sd['Final Predicted Fuel Consumption XGB After MC (kg/h)'],
                          lw=0.5, label=name)
            self.ax2.plot(sd['Ship Speed (knots)'],
                          sd['Final Predicted CO2 Emission XGB After MC (kg)'],
                          lw=0.5, label=name)
        # combined
    ...
    def generate_report(self):
        ...
        vals,fcp,co2p=self.last
        d={name:np.hypot(
                fcp-excel_data[name]['Final Predicted Fuel Consumption XGB After MC (kg/h)'].mean(),
                co2p-excel_data[name]['Final Predicted CO2 Emission XGB After MC (kg)'].mean()
            ) for name in sheet_names}
        s=sorted(d,key=d.get)
        text=(f"Input: {vals}\n"
              f"Pred FC: {fcp:.2f}, CO2: {co2p:.2f}\n"
              f"Closest: {s[0]}\nSecond Closest: {s[1]}\n")
